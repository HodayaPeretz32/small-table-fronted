...
import { useParams, useNavigate } from "react-router-dom";
import { getPackages, getPackageCategories, getPackageItems } from "../../api/packagesApi";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // מסנן packages לפי vendor
        const vendorPackages = allPackages.filter(
          (pkg) => pkg.vendor === Number(vendorId)
        );

        if (vendorPackages.length === 0) {
          setError("לא נמצאו חבילות לספק זה");
          setVendor(null);
          return;
        }

        // בנה את מבנה vendor עם packages
        const vendorData = {
          vendorId: Number(vendorId),
          vendorName: vendorPackages[0].vendor_name,
          packages: vendorPackages.map((pkg) => ({
            ...pkg,
            packageCategories: pkg.categories || [], // השתמש ב-categories מה-API
          })),
        };
        ...
        setError(
          `שגיאה בטעינת נתונים: ${err.response?.data?.detail || err.message}`
        );
      } finally {
        setLoading(false);
      }
